...
Tasks are split across an index file and one subfolder per task. The index stays small and navigable; each task lives in its own folder with its full definition and status.

```
.specs/features/[feature]/
├── spec.md
├── design.md
├── tasks.md                      # INDEX: execution plan, task list table, validation tables
└── tasks/
    ├── 01-create-user-use-case/
    │   └── TASK.md               # Full task definition + status (authoritative)
    ├── 02-validate-user-input/
    │   └── TASK.md
    └── ...
```
...
- Task subfolders use `NN-kebab-case-slug` (zero-padded number + descriptive slug).
- `NN` preserves execution order when sorted alphabetically.
- Slug describes the deliverable, not the task ID (e.g., `02-validate-user-input`, not `02-t2`).
- Each subfolder contains exactly one `TASK.md`.
- The folder can also hold task-specific scratch notes, diagrams, or reference snippets if needed.

**Why split?**

- **Index stays lean** — `tasks.md` is a navigable overview, not a 10k-token document.
- **Sub-agent context is surgical** — pass one `TASK.md` to a sub-agent, not the whole feature plan.
- **Status lives with the task** — each `TASK.md` carries its own Status field; index summarizes.
- **Parallel writes** — multiple sub-agents can update their own `TASK.md` without conflicting on one big file.

---

## Template: `.specs/[feature]/tasks.md` (Index)
...
| --- | -------------------------- | -------- | ---------- | ------------------------------------- |
| T1  | Create X Interface         | Draft    | None       | `tasks/01-create-x-interface/TASK.md` |
| T2  | Implement Y Service        | Draft    | T1         | `tasks/02-implement-y-service/TASK.md`|
| T3  | Create Z Component         | Draft    | T1         | `tasks/03-create-z-component/TASK.md` |
| T4  | Add A Feature to Y         | Draft    | T2, T3     | `tasks/04-add-a-feature-to-y/TASK.md` |

**Status values**: Draft | Ready | In Progress | Blocked | Done

> The Status column is a summary of each `TASK.md`'s Status field. Update both together when task status changes (see [implement.md](implement.md) Step 9).
...
2. Provide each sub-agent with its task definition (the relevant `tasks/NN-slug/TASK.md`) + context
3. Monitor sub-agent completion
4. Update both the task's `TASK.md` Status field and the index `tasks.md` summary table
5. Decide whether to proceed, fix, or escalate

---

## Template: `tasks/NN-slug/TASK.md` (Per Task)

One file per task. This is the authoritative definition — the index `tasks.md` only summarizes it.
...
**Reuses**: `src/existing/BaseInterface.ts` patterns
...
- [ ] [Specific, testable outcome]
- [ ] Gate check passes: `[command from Gate Check Commands in TESTING.md]`
...
- One `TASK.md` per task, inside its `NN-slug/` subfolder.
- `Status` here is authoritative. The index `tasks.md` table mirrors it.
- When marking Done, also record the commit SHA and gate result.
- Keep `What` to ONE sentence — if it needs a paragraph, the task is not atomic.
- `Reuses` references existing code patterns to copy — saves tokens and keeps consistency.
...
Before approving tasks, verify the execution diagram is consistent with the task definitions. These are independent artifacts that can drift — the diagram is drawn for visual clarity (in the index `tasks.md`) while task bodies are written for precision (in each `tasks/NN-slug/TASK.md`). Both must agree.

For each task, check:

| Task | Depends On (in TASK.md) | Diagram Shows | Status |
| ---- | ----------------------- | ------------- | ------ |
| T[N] | [deps from TASK.md] | [deps from diagram arrows] | ✅ Match or ❌ Mismatch |

**Rules:**

- Every `Depends on` in a `TASK.md` must have a corresponding arrow in the index diagram.
- Every arrow in the diagram must correspond to a `Depends on` in the target task's `TASK.md`.

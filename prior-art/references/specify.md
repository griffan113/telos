...
**Architectural gray areas → flag ADR need:** If a gray area decision touches architectural patterns, contracts between systems, service topology, database, messaging strategy, observability, or security, record in `context.md` that an ADR will be needed in the Design phase. Capture the proposed decision and rationale so Design can promote it directly. The ADR itself is NOT created here — Specify captures WHAT, not HOW. See SKILL.md "ADR Integration".

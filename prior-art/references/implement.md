...
From the index `tasks.md` (if exists) or from the execution plan above. User specifies ("implement T3") or suggest next available. The index points to the task's full definition at `tasks/NN-slug/TASK.md` — read that file for the authoritative What, Where, Done when, Tests, and Gate fields.

### 2. Verify Dependencies

If `tasks.md` exists, check dependencies in the index table (or in the task's `TASK.md`). If using inline plan, follow the order listed.
...
If the task includes tests (per the Tests field in `tasks/NN-slug/TASK.md` or TESTING.md coverage matrix):
...
3. **ADR check (architectural deviations only):** If the deviation (or the implementation itself) alters architectural patterns, contracts between systems, service topology, database, messaging strategy, observability, or security — AND meets the qualifiers in [adr-format.md](adr-format.md) (hard to reverse, surprising without context, real trade-off) — create an ADR in `docs/adr/` BEFORE committing. Reference `ADR-NNNN` in the commit body. See SKILL.md "ADR Integration".
4. Quick complexity check: "Would senior engineer flag this as overcomplicated?"
...
Mark the task complete in its `tasks/NN-slug/TASK.md` (Status field — authoritative) AND update the index `tasks.md` summary table to match. If the index has a Task List table, update the Status column there too. Update requirement traceability in spec.md if requirement IDs are used.
...
**Reading**: task definition from `tasks/NN-slug/TASK.md`

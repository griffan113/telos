...
Go through the index `tasks.md` Task List table (and each `tasks/NN-slug/TASK.md` Status field):
...
### 9. ADR Backfill Check

If UAT or final review reveals that an undocumented architectural decision was made during implementation (alters architectural patterns, contracts between systems, service topology, database, messaging strategy, observability, or security — and meets the qualifiers in [adr-format.md](adr-format.md)), backfill an ADR in `docs/adr/` BEFORE marking the feature complete. Reference the new `ADR-NNNN` in the validation report and in `.specs/project/STATE.md` Decisions section. See SKILL.md "ADR Integration".

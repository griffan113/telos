...
├── codebase/           # Architecture docs (brownfield: analyzed; greenfield: bootstrapped from decisions)
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── STRUCTURE.md
...
│       ├── tasks.md    # Index of tasks: execution plan, task list table (only for Large/Complex)
│       └── tasks/      # One subfolder per atomic task
│           └── NN-slug/
│               └── TASK.md  # Full task definition + status (authoritative)
└── quick/              # Ad-hoc tasks (quick mode)
    └── NNN-slug/
        ├── TASK.md
        └── SUMMARY.md

docs/adr/               # Architectural Decision Records (NNNN-slug.md, see references/adr-format.md)
```
...
2. Bootstrap architecture docs → seed `.specs/codebase/` from known decisions/ADRs (see [greenfield-bootstrap.md](references/greenfield-bootstrap.md)); keep updating as decisions land
3. For each feature → Specify → (Design) → (Tasks) → Execute (depth auto-sized)
...
- Codebase docs (when working in an existing project, or after bootstrapping them for a greenfield project)
...
- tasks.md (index, when executing tasks — plus the relevant `tasks/NN-slug/TASK.md` for the task being executed)
...
- The specific task definition from its `tasks/NN-slug/TASK.md` file (What, Where, Depends on, Reuses, Done when, Tests, Gate) — NOT the whole `tasks.md` index
...
The orchestrating agent uses this to update the task's `TASK.md` Status field and the index `tasks.md` summary table, traceability, and decide next steps.
...
| Bootstrap greenfield docs, document architecture decisions, seed codebase docs | [greenfield-bootstrap.md](references/greenfield-bootstrap.md) |
...
| Record architectural decision, create ADR | [adr-format.md](references/adr-format.md) |

## ADR Integration

ADRs (Architectural Decision Records) capture durable technical decisions in `docs/adr/`. Format and qualification criteria: [references/adr-format.md](references/adr-format.md).

### When to create an ADR

Create an ADR when a change alters ANY of these dimensions:

- architectural patterns
- contracts between systems
- service topology
- database (schema, technology, partitioning)
- messaging strategy (queues, events, buses)
- observability (logging, metrics, tracing)
- security (auth, authz, crypto, secrets)

PLUS the three qualifiers from `adr-format.md` must hold:
1. Hard to reverse
2. Surprising without context
3. Result of a real trade-off

If a decision is easy to reverse, not surprising, or had no real alternative, skip the ADR — record it inline in `design.md` Tech Decisions table or `STATE.md` instead.

### Where in the pipeline

| Phase | ADR action |
|---|---|
| **Specify (Discuss)** | If a gray area is architectural (touches the trigger list), flag in `context.md` that an ADR will be needed in Design; capture proposed decision and rationale |
| **Design** | **PRIMARY.** Promote decisions meeting the triggers from the `design.md` Tech Decisions table into formal ADRs in `docs/adr/`. Number by scanning `docs/adr/` for the highest existing number + 1 (format `NNNN-slug.md`) |
| **Tasks** | No new ADRs — just carry forward ADR references into task definitions when a task implements a decision recorded in an ADR |
| **Execute** | **SECONDARY (emergent).** If implementation reveals an architectural decision not captured in Design (e.g., a `SPEC_DEVIATION` marker touching the trigger list), create the ADR BEFORE committing the task. Reference the ADR number in the commit body |
| **Validate** | Backfill: if UAT or final review reveals an undocumented architectural decision was made, create the ADR before marking the feature complete |

### Linking

- `design.md` Tech Decisions table → row per ADR with `ADR-NNNN` reference
- `tasks/NN-slug/TASK.md` → cite `ADR-NNNN` in "Reuses" or "Done when" when the task implements an ADR decision
- Git commit body → `Refs ADR-NNNN` when the commit implements a decision recorded in an ADR
- `STATE.md` Decisions section → one-line entry per ADR created (date, ADR number, one-sentence summary)

### Existing ADRs

Before any Design or Execute decision touching the trigger list, scan `docs/adr/` for prior decisions on the same topic. ADRs are append-only — to reverse or supersede, create a new ADR with `Status: superseded by ADR-NNNN` in the old one.
...
Step 2: Project docs → README, docs/, docs/adr/, inline comments, .specs/codebase/

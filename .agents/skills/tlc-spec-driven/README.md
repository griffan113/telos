...
**Safety valve:** If listing inline steps reveals >5 steps or complex dependencies, the agent STOPS and creates a formal `tasks.md` index + per-task `TASK.md` files — acknowledging that the Tasks phase was wrongly skipped.
...
| `Break into tasks`, `Create tasks`        | Create tasks.md index + per-task TASK.md files  |
...
Agent: [Creates tasks.md index + per-task TASK.md files with 8 atomic tasks + parallel execution plan]
...
| `tasks.md`              | Granular task breakdown: index + per-task TASK.md methodology           |
